 ...
 */
@Slf4j
public class MySQL5Interceptor implements InstanceMethodsAroundInterceptor {
    @Override
    ...
    private String extractSql(EnhancedInstance instance, Method method) {
        String sql = null;
        try {
            Method asSql = method.getDeclaringClass().getMethod("asSql");
            sql = (String) asSql.invoke(instance);
            sql = sql.replace("\n", " ").replaceAll("( +)"," ").trim();
        } catch (Exception ex) {
